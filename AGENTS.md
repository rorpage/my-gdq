...
- `api/watchlist.ts`: stateless keyword-match endpoint for external callers (cron, a launcher app, curl). Takes `?keywords=a,b,c&tz=IANA/Zone&hours=24`, auto-detects the current event, and returns runs matching the keywords that are live now or start within the lookahead window.
...
- **Keywords stay stateless server-side.** The web app's watchlist lives only in the browser's `localStorage`; there is no account system to sync it against. `api/watchlist.ts` mirrors that: the caller passes its own keyword list as a query param on every request rather than the server remembering one. This keeps the endpoint a pure function of its inputs, consistent with the no-backend-state design.
...
- **Rolling lookahead window, not "today."** `api/watchlist.ts` originally bucketed runs by local calendar day, but the tracker's timestamps are in the event's own zone (observed as `-05:00`/Central for SGDQ 2026), not the caller's. Converting a late-night Central run to a caller further east can genuinely push it past local midnight, so a calendar-day filter silently dropped runs that were clearly "tonight" to a human. `isUpcomingWithin()` (`js/schedule-utils.js`) replaces the day bucket with "hasn't ended yet and starts within `hours` from now" (default 24). `tz` is now used only to render each match's `starttime_local` for display, not to gate matches.
...
### 2026-07-08: Watchlist endpoint for external alerts

User asked whether filtering happens in the UI or the API (it is entirely client-side, see `filterRuns()`), then wanted a way to ask "any of my games today?" from outside the browser and get alerted. Since keywords only ever lived in `localStorage`, added `api/watchlist.ts`: a stateless endpoint that takes `?keywords=a,b,c&tz=IANA/Zone`, auto-detects the current event via the new `pickCurrentEvent()` pure function, and returns today's runs (in the caller's local day) matching those keywords. Extracted `fetchAllPages()` into `api/_lib.ts` so `events.ts`, `runs.ts`, and the new endpoint share one pagination implementation instead of three copies. Tests: 3 new cases for `pickCurrentEvent()`, 14 passing total. First pass also added a macOS-only Alfred script for the notification side, but the user isn't on a Mac, so that was dropped; the endpoint itself is the deliverable, and the client (Alfred, cron, whatever) is up to whatever the user is actually running. Not verified against the live tracker for the same egress reason as the previous entry; the "current event" heuristic and the endpoint's shape should be sanity-checked after deploy.

Deployed to production, then user reported a keyword ("banjo") not catching a run ("Banjo-Tooie") they expected. Traced it to `api/watchlist.ts`'s "today, caller's local day" filter: the raw tracker payload confirmed `starttime` values are in the event's own zone (`-05:00`, Central for SGDQ 2026), not the caller's. Converting Central 11:39 PM to a caller in `America/Indiana/Indianapolis` (Eastern) correctly lands on 12:39 AM, but that pushed it past local midnight into "tomorrow," so the calendar-day filter dropped it, even though no timezone conversion was actually wrong. Replaced the day-bucket filter with `isUpcomingWithin()`, a rolling lookahead window ("hasn't ended, starts within `hours` from now, default 24") that has no midnight cliff. `tz` now only formats each match's new `starttime_local` field rather than gating which runs match. Added `hours` as an optional query param. Tests: 5 new cases for `isUpcomingWithin()`, 19 passing total.
