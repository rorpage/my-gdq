...
- `api/watchlist.ts`: stateless keyword-match endpoint for external callers (cron, a launcher app, curl). Takes `?keywords=a,b,c&tz=IANA/Zone`, auto-detects the current event, and returns today's matching runs.
...
- **"Today" is the caller's local day, not the server's.** `api/watchlist.ts` takes a `?tz=IANA/Zone` param and buckets runs by local calendar day in that zone (`Intl.DateTimeFormat('en-CA', ...)` for a sortable `YYYY-MM-DD` key), matching the `groupRunsByDay()` approach already used client-side. Omitting `tz` defaults to UTC, which is wrong for most callers, so real usage should always pass it.
